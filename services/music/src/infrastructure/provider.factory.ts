 ...
 * El orden define la prioridad al intercalar resultados: el catálogo
 * comercial (Deezer) primero porque es lo que el usuario reconoce.
 */
export function createProviders(env: NodeJS.ProcessEnv): MusicProvider[] {
  const providers: MusicProvider[] = [new DeezerAdapter()];
